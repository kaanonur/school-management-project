                                            ...
                                            <div class="col-md-12">
                                                <table class="table table-bordered table-striped w-full">
                                                    ...
                                                    @foreach($data['employees'] as $key => $employee)
                                                        <tr id="div_{{ $employee->id }}" class="text-center">
                                                            <input type="hidden" name="employee_id[]" value="{{ $employee->id }}">
                                                            <td>{{ $key + 1 }}</td>
                                                            <td>{{ $employee->name }}</td>
                                                            <td colspan="3">
                                                                <div class="switch-toggle switch-3 switch-candy">
                                                                    <input name="attend_status_{{$key}}" type="radio" id="present_{{$key}}" value="Present" checked="checked">
                                                                    <label for="present_{{$key}}">Present</label>

                                                                    <input name="attend_status_{{$key}}" type="radio" value="Leave" id="leave_{{$key}}">
                                                                    <label for="leave_{{$key}}">Leave</label>

                                                                    <input name="attend_status_{{$key}}" type="radio" value="Absent" id="absent_{{$key}}">
